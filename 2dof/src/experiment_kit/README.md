...
colcon build --packages-select experiment_kit
source install/setup.bash
```

### 2. Launch robot in Gazebo (with automatic motion)
```bash
ros2 launch experiment_kit spawn_robot.launch.py
```
...
```bash
ros2 run experiment_kit manual_joint_control
...
```
[Err] [SystemPaths.cc:378] Unable to find file with URI [model://experiment_kit/meshes/...]
```
...
- `experiment_kit/joint_position_commander.py` - Automatic sine motion
- `experiment_kit/manual_joint_control.py` - Interactive control
