...
def generate_launch_description():
    ...
    # Get URDF via xacro
    pkg_experiment_kit = FindPackageShare('experiment_kit')
    
    robot_description_content = Command(
        [
            PathJoinSubstitution([FindExecutable(name="xacro")]),
            " ",
            PathJoinSubstitution([pkg_experiment_kit, "urdf", "link1_vertical.xacro"]),
        ]
    )
    ...
    world_file = PathJoinSubstitution([pkg_experiment_kit, "worlds", "hexapod.sdf"])
    ...
    pd_step_commander_node = Node(
        ...
        executable="pd_step_commander_link1",
        name="pd_step_controller_link1",
        output="screen",
        parameters=[{
